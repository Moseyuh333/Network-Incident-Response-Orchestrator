...
export interface ProtectedCidr {
  /** Human-readable label surfaced in deny messages. */
  label: string;
  /** IPv4 address string (no CIDR suffix for exact-match entries). */
  network: string;
  /** Prefix length; 32 means "exact match only". */
  prefix: number;
}

const PROTECTED_ENTRIES: ProtectedCidr[] = [
 ...
 */
export function findProtectedCidr(target: string): ProtectedCidr | null {
  if (!target) return null;
  const targetInt = ipv4ToInt(target);
  if (targetInt === null) return null;

  for (const entry of PROTECTED_ENTRIES) {
    const networkInt = ipv4ToInt(entry.network);
    if (networkInt === null) continue;
    if (entry.prefix === 0) return entry;
    if (entry.prefix > 32) continue;
